...
template <typename T>
struct Node { Node(T val) { this->val = val; } Node<T> *left, *right; T val; };
...
public:
	...
	Node<T> *build(const int inorder[], const int preorder[], int pl, int pr, int il, int ir) {
		int len = pr - pl + 1; if (len <= 0) return NULL;

		int i = il; Node<T> *rt = new Node<T>(preorder[pl]);
...
template <typename T>
bool is_balance(Node<T> *root) { return (count_max<T>(root) - count_min<T>(root)) <= 1; }
...
int main() {
	...
	for (int i = 0; i < pro.length(); ++i) {
		preorder[i] = pro[i] - 'A'; inorder[i] = ino[i] - 'A';
	}

	Tree<int> *tr = new Tree<int>(inorder, ino.length(), preorder, pro.length());
	tr->inord_traverse(tr->root()); printf("\n");
	tr->preord_traverse(tr->root()); printf("\n");
