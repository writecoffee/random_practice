...
string *get_binary(string *str) {
	...
	if (str_dec_part.length() > 32) { return NULL; }

	// generate integer part
	int int_part = 0; string::reverse_iterator i_rit; int i_step;
	...
	double dec_part = 0.0; string::iterator d_it; double d_step;
	...
	while (dec_part > 0.0) {
		if (dec_part == 1) { str_dp_bin.push_back('1'); break; }
		double r = dec_part * 2;
		if (r >= 1) { str_dp_bin.push_back('1'); dec_part = r - 1; }
		else { str_dp_bin.push_back('0'); dec_part = r; }
	}
...
int main() {
	srand(time(0)); int i_len = 5, d_len = 5; string str, *result = NULL;
	...
	str.push_back('.');
	for (int j = 0; j < d_len; ++j) { str.push_back('0' + rand() % 10); }
