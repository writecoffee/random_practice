...
void mark_depth(Node<T> *cr, int d, multimap<int, Node<T> *> *mm) {
    assert(cr);
    if (cr) {
        mm->insert(pair<int, Node<T> *>(d, cr));
    }
    if (cr->l) {
        mark_depth(cr->l, d + 1, mm);
    }
    if (cr->r) {
        mark_depth(cr->r, d + 1, mm);
    }
}
...
Node<T> *build_binary_tree(int height) {
    ...
    queue<Node<T> *> r, s, *q = &r, *p = &s;
    int h = 1;
    Node<T> *root = new Node<T>(rand() % 100);
    q->push(root);
    ...
    while (1) {
        ...
        while (!q->empty()) {
            ...
            // left child creates
            {
                t->l = new Node<T>(rand() % 100);
                p->push(t->l);
            }

            // right child creates
            if (rand() % 2) {
                t->r = new Node<T>(rand() % 100);
                p->push(t->r);
            }
        }
