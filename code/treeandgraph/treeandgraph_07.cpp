...
struct Node {
    ...
    Node(const Node<T> &cp) : left(NULL), right(NULL), val(cp.val), depth(cp.depth) {
        if (cp.left) {
            this->left = new Node<T>(*cp.left);
        }
        if (cp.right) {
            this->right = new Node<T>(*cp.right);
        }
    }
...
bool sub(const Node<T> *p, const Node<T> *q) {
    if (_N(p)) {
        return false;    // source tree ends, matching job finish
    }
    else if (p->val == q->val) {
        if (match(p, q)) {
            return true;
        ...
        return (sub(p->left, q) || sub(p->right, q));
    }
}
...
bool match(const Node<T> *p, const Node<T> *q) {
    if (_N(p) && _N(q)) {
        return true;    // both end, finish match
    }
    else if (_N(p) || _N(q)) {
        return false;    // one side end
    }
    else if (p->val != q->val) {
...
Node<T> *build_tree(const int height, Node<T> **sub_t) {
    ...
    int h = 1;
    Node<T> *root = new Node<T>(rand() % 100, 0);
    q->push(root);
...
bool tree_trunc(Node<T> *t, const int d) {
    ...
    bool ret = false;
    if (t->left) {
        if (tree_trunc(t->left, d)) {
            ret = true;
        }
    }
    if (t->right) {
        if (tree_trunc(t->right, d)) {
            ret = true;
        }
    }
